    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                <Input placeholder="Search all leases..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-sm" />
            ...
            <div className="border rounded-md">
                <Table>
                    ...
                    <TableBody>
                        {isLoading && <TableRow><TableCell colSpan={6} className="h-24 text-center">Loading DHCP leases...</TableCell></TableRow>}
                        {error && <TableRow><TableCell colSpan={6} className="h-24 text-center text-destructive">Failed to load DHCP leases.</TableCell></TableRow>}
                        {!isLoading && filteredLeases.length === 0 && <TableRow><TableCell colSpan={6} className="h-24 text-center">No DHCP leases found.</TableCell></TableRow>}
                             ...
                             <TableRow key={lease['.id']} data-state={selectedLeases.some(l => l['.id'] === lease['.id']) && "selected"}>
                                ...
                                <TableCell className="text-center">
                                    {lease.dynamic === 'true' && (
                                     ...
                                     <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => setLeasesToDelete([lease])}>
