...
export default function StaticLeasesTab({ token }) {
  ...
  const staticLeases = useMemo(() => {
    ...
    return staticOnly.filter((lease) => Object.values(lease).some((v) => String(v).toLowerCase().includes(q)));
            ...
            {error && (
              <TableRow>
                <TableCell colSpan={5} className="h-24 text-center text-destructive">
                  Failed to load static leases.
                </TableCell>
              </TableRow>
            )}
            {!isLoading && staticLeases.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="h-24 text-center">
                  No static leases found.
                </TableCell>
              </TableRow>
            )}
